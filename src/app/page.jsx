...
export default function Home() {
  ...
  return (
    <main className="min-h-screen flex flex-col relative">
      ...
      <motion.nav initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.5, ease: "easeOut" }} className="absolute top-0 w-full z-50 flex justify-between items-center p-6 lg:px-12 bg-transparent text-white">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1.5, delay: 0.8, ease: "easeOut" }} className="relative w-48 h-8 md:h-10 hover:opacity-80 transition-opacity cursor-pointer">
      ...
      <section className="relative h-screen flex flex-col justify-center items-center text-center p-6 overflow-hidden">
        <Image src="/images/portfolio/hero-bg.jpg" alt="Stangherlin Arquitetura" fill priority sizes="100vw" className="object-cover object-center -z-10" />
        <div className="absolute inset-0 bg-black/50 z-0"></div>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.5, ease: "easeOut" }} className="relative z-10 flex flex-col items-center mt-16 w-full px-4">
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-light text-white mb-6 tracking-wide drop-shadow-lg">Arquitetura <span className="font-semibold">&</span> Inteligência Urbana</h1>
          <p className="text-lg md:text-xl text-gray-200 max-w-2xl mb-10 font-light drop-shadow-md">Projetos de alto padrão e análises estratégicas para maximização de potencial construtivo.</p>
      ...
      <section id="manifesto" className="bg-white text-black py-24 lg:py-32 px-6 lg:px-12">
        <div className="max-w-6xl mx-auto">
          <div className="mb-20 text-center lg:text-left">
            <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1 }} className="text-xs uppercase tracking-[0.3em] text-gray-400 mb-6">O Manifesto Stangherlin</motion.h2>
      ...
      <section className="bg-[#0a0a0a] text-white py-24 px-6 lg:px-12 border-t border-gray-800" id="portfolio">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-center items-center md:divide-x divide-gray-800 text-center gap-12 md:gap-0">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="px-10 flex flex-col items-center w-full md:w-1/3">
            <span className="text-4xl md:text-5xl lg:text-6xl font-light mb-3 text-white">+3 Milhões</span>
            <span className="text-xs uppercase tracking-[0.2em] text-gray-500">De m² Projetados</span>
          </motion.div>
      ...
      <section id="viabilidade-cta" className="bg-gray-900 text-white p-12 lg:p-32 text-center">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto mb-10 text-lg font-light">Desenvolvemos uma ferramenta exclusiva para análises rápidas de zoneamento e potencial construtivo. Tome decisões de investimento com precisão geométrica.</p>
          <a href="/viabilidade" className="inline-block bg-white text-black px-12 py-5 uppercase tracking-widest text-sm font-bold shadow-[0px_0px_15px_rgba(255,255,255,0.25)] hover:bg-gray-100 transition-all hover:scale-105 active:scale-95">Descobrir Potencial do Terreno</a>
        </div>
      </section>
